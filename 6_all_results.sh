...
#SBATCH --output=6_eval_odd.txt
#SBATCH --error=6_eval_odd.txt
...
OUT_DIR="${WORK_DIR}/result/table-odd"
mkdir -p "${OUT_DIR}"
...
if [ -f "${WORK_DIR}/6_all_results.py" ]; then
    echo "  FOUND   ${WORK_DIR}/6_all_results.py"
else
    echo "  MISSING ${WORK_DIR}/6_all_results.py"
    exit 1
fi
...
runpy.run_path('${WORK_DIR}/6_all_results.py', run_name='__main__')
PYEOF
