...
#    Set empty string for models you don't have
# -----------------------------------------------------------------------------
...
# Only add optional flags if the file exists AND path is not empty
optional_flags = [
...
for flag, path in optional_flags:
    if path and os.path.isfile(path):
        ...
        print(f"  [argv] skipping {flag} (file not found or empty: {path})")

sys.argv = argv
print(f"  [argv] {' '.join(argv)}\n")

runpy.run_path('${WORK_DIR}/4-compare_models.py', run_name='__main__')
PYEOF
