...
if [ -f "${WORK_DIR}/6_all_results_odd.py" ]; then
    echo "  FOUND   ${WORK_DIR}/6_all_results_odd.py"
else
    echo "  MISSING ${WORK_DIR}/6_all_results_odd.py"
    exit 1
fi
...
runpy.run_path('${WORK_DIR}/6_all_results_odd.py', run_name='__main__')
PYEOF
