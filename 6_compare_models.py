...
=================================
Compares all neural models + baselines. Outputs 3 simple text tables + figures.
...
# ═════════════════════════════════════════════════════════════
# SIMPLE TEXT TABLE GENERATION (NO LATEX)
# ═════════════════════════════════════════════════════════════

def write_text_table_1(results, out_path, models_in_table, val_graphs, mc_runs, N):
    """
    Table 1: Cross-topology RMSE — BA / ER / WS, hub placement, all Z.
    Simple text format.
    """
    lines = []
    lines.append("=" * 120)
    ...
    for Z in ALL_Z:
        row = f"{Z:>6}"
        for topo in ["ba", "er", "ws"]:
            # Find best in this row for this topology
            vals = {}
            for m in models_in_table:
...
def write_text_table_2(results, out_path, models_in_table, val_graphs, mc_runs, N):
    """
    Table 2: Zealot placement — BA, Hub / Bridge / Random × Z.
    Simple text format.
    """
...
def write_text_table_3(results_gen, out_path, models_in_table, val_graphs, mc_runs):
    """
    Table 3: Size generalization — all topologies, hub placement, Z=8.
    Simple text format.
    """
    lines = []
    lines.append("=" * 120)
    ...
    for n_val in SIZE_LIST:
        ...
        for topo in ["ba", "er", "ws"]:
            vals = {}
            for m in models_in_table:
                mean, _ = results_gen.get((topo, n_val, m), (float("nan"), 0))
                vals[m] = mean
            best = _best_in_row(vals)
            for m in models_in_table:
                mean, std = results_gen.get((topo, n_val, m), (float("nan"), float("nan")))
                if np.isnan(mean):
                    cell = "      N/A      "
                else:
                    is_best = (abs(mean - best) < 1e-6)
                    mark_cell = "*" if is_best else " "
